...
[kubelet](/docs/admin/kubelet/)使用liveness probe（存活探针）来确定何时重启容器。例如，当应用程序处于运行状态但无法做进一步操作，liveness探针将捕获到deadlock，重启处于该状态下的容器，使应用程序在存在bug的情况下依然能够继续运行下去。
...
可以使用命名的[ContainerPort](/docs/api-reference/v1.6/#containerport-v1-core)作为HTTP或TCP liveness检查：
